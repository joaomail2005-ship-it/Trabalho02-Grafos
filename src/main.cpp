...
void imprimirUso(const char *nomePrograma) {
  ...
  cout << nomePrograma
       << " <caminho.stp> randomizado <alpha> <iteracoes> [semente]\n";
  cout << nomePrograma
       << " <caminho.stp> reativo <iteracoes> <bloco> [semente]\n";
}
...
int main(int argc, char *argv[]) {
  ...
  if (modo == "guloso" && argc == 4)
    semente = stoi(argv[3]);
  else if (modo == "randomizado" && argc == 6)
    semente = stoi(argv[5]);
  else if (modo == "reativo" && argc == 6)
    semente = stoi(argv[5]);
  else {
    // Se não foi informada, gera pela data/hora atual
    semente = static_cast<unsigned int>(
  ...
  if (modo == "guloso") {
    ...
    if (argc < 5) {
      cout << "Erro: Faltam parametros.\n";
      return 1;
    }

    alpha_usado = stod(argv[3]);
    iteracoes_totais = stoi(argv[4]);
    cout << "Executando Randomizado (Alpha: " << alpha_usado
    ...
    bloco = stoi(argv[4]);
    cout << "Executando Reativo (Iteracoes: " << iteracoes_totais
         << " | Bloco: " << bloco << ")...\n";
    ...
    for (int i = 1; i <= iteracoes_totais; ++i) {
      discrete_distribution<int> roleta(probabilidades.begin(),
                                        probabilidades.end());
  ...
  if (arquivoNovo) {
    ...
    arquivoLog << "Data_Hora,Instancia,Algoritmo,Alpha,Iteracoes,Tamanho_Bloco,"
                  "Semente,Tempo_s,Melhor_Custo\n";
  }

  // Instância apenas pelo nome (remover pastas se necessário, aqui salva o
  // caminho completo)
  arquivoLog << getDataHoraAtual() << "," << caminhoArquivo << "," << modo
             << "," << alpha_usado << "," << iteracoes_totais << "," << bloco
             << "," << semente << "," << fixed << setprecision(4) << tempo_total
